...
$query_total_users = "SELECT COUNT(*) AS total_users FROM users WHERE user_type = 'u'";
$result_total_users = mysqli_query($conn, $query_total_users);
$total_users = ($result_total_users && mysqli_num_rows($result_total_users) === 1) ? mysqli_fetch_assoc($result_total_users)['total_users'] : 0;
...
$total_students = ($result_total_students && mysqli_num_rows($result_total_students) === 1) ? mysqli_fetch_assoc($result_total_students)['total_students'] : 0;

$admin_id = $_SESSION['user_id'];

$query_admin = "SELECT full_name, image FROM users WHERE user_id = '$admin_id' AND user_type = 'a'";
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td><button class='update-btn' data-id='" . $row['students_id'] . "'>Update</button>
    ...
    <div id="updateStudentModal" class="modal">
        <div class="modal-content">
            ...
            <form id="updateStudentForm" method="POST" action="update_student.php">
                ...
                <input type="text" id="update_student_name" name="student_name" required>
                <label for="update_contact_number">Contact Number:</label>
                <input type="text" id="update_contact_number" name="contact_number" required>
                ...
                <label for="update_birth_date">Birth Date:</label>
                <input type="date" id="update_birth_date" name="birth_date" required>
                <label for="update_course">Course:</label>
                <select id="update_course" name="course" required>
                    <option value="BSIT">BSIT</option>
                    <option value="BSIS">BSIS</option>
                    <option value="BSCS">BSCS</option>
                    <option value="BSIT - Animation">BSIT - Animation</option>
                ...
                <select id="update_year" name="year" required>
                    <option value="1">1</option>
                    <option value="2">2</option>
                    <option value="3">3</option>
                    <option value="4">4</option>
                ...
                <label for="update_block">Block:</label>
                <select id="update_block" name="block" required>
                    <option value="A">A</option>
                    <option value="B">B</option>
                    <option value="C">C</option>
                    <option value="D">D</option>
                ...
                <button type="submit" class="modal-submit-btn">Update Student</button>
            </form>
        </div>
    </div>
